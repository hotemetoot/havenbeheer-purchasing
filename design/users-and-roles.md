...
| Dept owner (via `main_approver` field, not a role) | the user on `departments.main_approver` | approve / return / reject PRs at the Dept stage; edit PR content while in their queue (D16); when a submitter reassigns the dept stage via `custom_approver` (D36), receives an in-app FYI notification instead of the approval task (the chosen approver gets the task) |
