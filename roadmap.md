...
| 015 | "PR approved" notifications — on final approval, in-app notify the requester always + head of procurement only on the director path (not when Procurement is final, not on board path) (D50) | ◑ built 2026-06-24 — workflow `cv237r8h7k9` rev `371864957943808` active; 3 notification nodes (`hy95mz4oo5f`/`dproua9530i`/`p53qqltz9v2`). Live walkthrough A1–A5 pending. See [chunks/015-pr-approved-notifications.md](chunks/015-pr-approved-notifications.md) |
