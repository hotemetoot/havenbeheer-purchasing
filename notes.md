...
- **Workflow revisions:** once a workflow has executed, it can't be edited in place — node edits 400 with "Nodes in executed workflow could not be reconfigured." Create a new same-key revision, edit while disabled, enable, disable the predecessor.
  - **`revision` needs `key` at the TOP LEVEL of `--filter`, NOT wrapped in `$and`.** `nb api workflow workflows revision --filter '{"$and":[{"key":"..."}]}'` does NOT read the key — it falls to the "otherwise" branch and creates an **independent copy with a brand-new key** (a stray disabled workflow), not a new version of the same lineage. Use `--filter '{"key":"<key>"}'` flat. (Hit 2026-07-04 revising the Receive guard `mhfp4d15uee` — made stray `rvhcisgzsv2`, had to delete it and redo. This one place breaks the "always wrap filters in `$and`" rule.)
  - **The revision copy is not always faithful — verify copied node configs against the source.** Revising the Receive guard, the duplicated condition node silently **dropped one `and` calculation clause** (`status != issued`) present in the source. Node **keys** were preserved (cross-references like `{{$jobsMapByNodeKey.567k7v8jzsi...}}` still resolve), but a clause vanished. Always diff the new version's node config against the old before enabling; when you're overwriting the node anyway, set the whole clause list explicitly rather than trusting the copy.
