...
**Correction 2026-07-02:** Alexander clarified these personas are themselves dev/test-only (nothing here is a real production identity — this is local dev, and go-live migration moves only settings/ACL/workflows, never data or these accounts). They're fine for the test suite to use directly, including changing their passwords to a shared value (`nbtest`) — the constraint above about "must not assume/touch/clean up" was more caution than necessary and can be relaxed. Not yet done: no plan.yaml fixture currently references them (R13/R14/R16 used fresh synthetic `test_director_a@test.local`-style users instead, to avoid this question entirely for that batch). Needed before R12/R15/R17–R24 can be drafted — see "Approval-chain test fixtures" below.

## Approval-chain test fixtures — needed for R12/R15/R17–R24 (blocked, 2026-07-02)
...
**This is solvable, not a hard blocker** — corrected an earlier wrong claim that "approval chains can't be tested" (see `nb-project-suite`'s own `HANDOFF.md` for the cross-project correction). The mechanism: `approvalRecords:submit` is a real REST action (`{filterByTk: <approvalRecordId>, values: {status: 2}}` for approve, `status: -1` for reject — constants from `@nocobase/plugin-workflow-approval`'s `constants.js`), and `runner.py`'s `Client.act()` is already generic enough to call it. The blocker is narrower: `runner.py`'s declarative `plan.yaml` schema has no case type that expresses "sign in as an approver, submit a decision" yet — a `nb-project-suite` change, not a Havenbeheer one.

Once that case type exists, use the real personas above (`dana.director`=id 12, `pat.procurement`=id 11, `fiona.finance`=id 14, `oliver.owner`=id 10 — all already set as the relevant `departments.main_approver`, see D40) as the approver fixtures, with the shared `nbtest` password. Don't create new synthetic approver users — the whole point of driving the real chain is exercising the real assignee-matching logic in `approvalRecords:submit`'s `validateSubmit`, which checks the record's actual assigned `userId`.
