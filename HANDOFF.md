...
**Runner gotcha found this session:** `run --seed` re-updates *all* fixture users (including existing ones) and this disturbed an existing user's auth mid-run — case R12 flaked with an HTTP 401 on the same run that seeded a new user, then passed 14/14 on a plain `run`. Workaround: to add a new fixture user, run `--seed` once to create it, then run *without* `--seed`. Flagged as a background task to fix in the shared runner (`task_d2c5b57d`).
