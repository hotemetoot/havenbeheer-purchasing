...
**Status:** built + suite-green 2026-07-18, pending Alexander's UI confirmation (go-live §1.2b). Not re-proven this session: that Generate PO still populates the five removed fields — precedent is D46 removing the same fields from the update whitelist with no effect on the workflow, since workflow create nodes bypass ACL. Rollback = disable `376238895661056` and re-add the five field names to the create whitelist.
