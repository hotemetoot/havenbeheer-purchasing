...
**Affects:** New test rule R46 with 4 cases (three refusals — plain id, nested record, no parent — plus a nested-shape allow on a draft PO, so the fix cannot read green by blocking everything). The nested case is the regression guard: HTTP 500 is not in the runner's `DENY_CODES`, so it fails loudly if the direct read ever returns. Confirmed independently by a parallel session hitting the same 500. The sibling guards `c9c14tyn876` (line budget ceiling) and `eiscjvwiqr6` (line freeze) read `values.*` in the same style and have **not** been checked for this shape — worth a pass before go-live.
**Status:** built + verified live 2026-07-19 across all seven payload/status combinations. Test debris created during the failed attempt (12 po_lines, 3 of them on closed PO-26-0468) deleted with Alexander's approval. **Suite not yet re-run** — `tests/.env.test` is absent from the worktree. Rollback = re-enable `369880930516992`, disable `376333087145984`.
