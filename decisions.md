...
**Status:** superseded by D88 (2026-07-19) — the recipient rule below is replaced; the rest stays historically valid. Originally effective (user walkthrough A1–A5 passed 2026-06-24).
...
## D80 — Pruned the 130-execution status-0 backlog (2026-07-16)
**Decision:** Deleted all 130 pending (status-0) workflow executions (oldest 2026-05-12), the backlog D79 queued for pruning. Classified first, approved by Alexander: 66 on deleted/disabled superseded revisions, 64 on current PR/Project Approval but with their triggering records deleted (test-fixture orphans), 0 tied to live records — every existing PR/project is `approved`, none mid-approval.
**Why:** the backlog kept the dispatcher's pending list non-empty, the enabler of the D79 double-processing race.
**How to apply:** `executions:destroy` silently skips status-0 rows (server-side `$ne: STARTED` filter, returns `data: 0`); cancel each by tk first, then destroy. Cancel 500s on approval orphans ("Cannot destructure property 'approval'") but the abort still lands, so destroy succeeds. Full mechanics in nb-bootstrap gotchas. Pruning reset the `executed` counters (known gotcha).
**Affects:** nothing structural. Open issue: nb-test teardown deletes fixture PRs/projects but not their pending approval executions — each suite run leaves ~9 new status-0 orphans (verified after the post-prune run). Harmless with D79's `deleteExecutionOnStatus: []`, but the backlog will regrow; fix belongs in the nb-test runner's teardown.
**Status:** done 2026-07-16. Post-prune suite 77/77 green; log shows "fetched from db" only for normal async approval dispatch, zero `SequelizeInstanceError`/"hangs".

## D81 — Maintenance pass: 53 superseded workflow revisions + pre-July execution history deleted (2026-07-16)
**Decision:** First nb-maintain pass. Per Alexander's rule "keep the current version plus two predecessors per lineage," deleted 53 superseded revisions (all `enabled:false`, non-current; 35 of them PR Approval, 7 Generate PO, 4 Send PO, rest scattered) — workflows table 112 → 59 rows, all 27 lineages verified with exactly one current. Pruned all pre-2026-07-01 terminal executions (513 status-1 + 134 other terminal; status-0 untouched). Pre-deletion `.nbdata` backup `havenbeheer_2026-07-16_0807.nbdata` on iCloud.
**Why:** the keep-everything-as-rollback stance (2026-07-05) had grown to 79 stale rows; two-predecessor depth keeps real rollback while cutting the pile. Old execution history was dead weight (D79/D80 made history prunable by design).
**How to apply:** future passes repeat the same rule: per lineage keep current + 2 newest predecessors, delete older; prune terminal executions older than ~2 weeks. Filter-guard every destroy with `enabled:false` + `current:{"$ne":true}`.
**Affects:** rollback depth for all workflow lineages is now 2 versions (deeper rollback = restore a backup). This maintenance pass also refreshed HANDOFF.md (stale 2026-07-14 warnings), merged D80's stranded worktree branch, and removed that worktree.
**Status:** done 2026-07-16. Post-cleanup `runner.py acl` 34/34 green; status-0 count 9 (today's suite-run orphans, see D80's open issue).

## D82 — All PO-side lock guards now exempt admin (retro pass, 2026-07-16)
**Decision:** The three PO lock guards got the D71/D63 admin-exempt head (caller root/admin lookup → run the lock check only when the caller is not admin): PO Immutability (`xvcsdv07c5j`, new version `375761699536896`), PO Line Immutability (`f3dkb37te22`, new `375761900863488`), PO Line Destroy (`v61hc3ou3pa`, new `375767284252672`). Predecessors disabled as rollback (`368747670863872` / `368747750555648` / `373256900640768`). Guard A (PRs, D71) and the projects guard (D63) already had it — every lock guard is now admin-exempt.
**Why:** Alexander's 2026-07-16 retrospective decision: admin may delete/edit PRs, POs, and PO lines for cleanup, so guards never have to be disabled for teardown. Business roles are unchanged.
**How to apply:** Every revision copy was diffed clean before editing; `deleteExecutionOnStatus` stays `[]` (D79). Deliberately NOT exempted: the rule-checking guards (Create PO, line-create/budget ceilings, Close, Receive) — they aren't cleanup blockers and their checks should apply to admin too. Now a standing CLAUDE.md rule: new interception guards ship with the admin-exempt head by default.
**Affects:** nb-test teardown (terminal PO/line debris now removable by admin); workflows-explained.md §4 (updated same session).
**Status:** done + live-verified 2026-07-16 — 6/6 probes (admin update on completed PO + line and destroy pass-through OK; Pat denied with the right lock messages) and full suite 77/77 green. Rollback = disable the three new versions, re-enable the predecessors.

## D83 — PO lines_total recompute replaced by an async workflow that reloads the line (2026-07-18)
**Decision:** The sync `po_lines` create/update recompute (key `5ukanitoy74`, was rev `372365019643904`) is disabled, replaced by a NEW async workflow `1ugka88lngm` (id `376139238998016`, same title/trigger: po_lines create/update, changed fields quantity_ordered/unit_price). Chain: query "Reload line with PO" (`mto9x0s1rz2`, by `{{$context.data.id}}`, appends purchase_order) → aggregate sum (`fix936xyxau`, filter on the RELOADED `purchaseOrderId`) → update `lines_total` (`ue415sq2s0e`). No node reads the trigger payload's FK anymore.
**Why:** 016's parked bug — on import/association create the collection event fires before the PO foreign key lands, so the old sync workflow saw `purchaseOrderId = null` and never recomputed (`lines_total` stale until a manual line touch). Fix needs both halves: async (runs post-commit, when the FK exists in the DB) and the reload (the payload snapshot never has it). `sync` is immutable after creation — server rejects flipping it on update AND on revision — so this had to be a new workflow, not a revision of the old lineage.
**How to apply:** old workflow kept disabled as rollback. Guard payload shapes learned en route: Create-PO guard reads `values.purchase_request` as a bare id; the D47 line budget guard reads `values.purchase_order` as a bare id (assoc-create without it → guard crashes with a math TypeError on null — pre-existing, unchanged).
**Affects:** closes 016's parked follow-up. Import now updates `lines_total` (per-row, post-commit) — UI import re-test still pending (016 B-cases). tests/plan.yaml: no rule cites the old workflow; a lines_total recompute rule is a proposed addition.
**Status:** built + live-verified 2026-07-18 (fixture PR→PO→line: create wrote 200, quantity edit wrote 300, execution jobs show the reloaded FK; fixtures deleted). Suite 77/77 green post-swap. Rollback = disable `1ugka88lngm`, re-enable `372365019643904`.

## D84 — Every workflow carries a category + description; Guard A's dead `cancelled` clause dropped (2026-07-18)
**Decision:** All 28 current workflows now carry exactly one category — Guards (13), Actions (6), Recompute (6), Approval (3) — and a plain-English description on the workflow record, written from live node reads. Two new categories created: Actions (`376198103957504`), Recompute (`376198103957505`). Two duplication-leftover titles fixed: "Close PO copy" → "Close PO" (`369914944225280`), "PO Close Guard … copy copy" → "Guard: PO Close — block close on non-closeable PO" (`369970388729856`). Separately, Guard A (`496ookqmg01`) got new version `376198840057856` dropping the `cancelled` clause D68 left dead; predecessor `373846756098048` disabled as rollback.
**Why:** 28 workflows with bare titles were no longer scannable, and several titles lied about their contents. The `cancelled` clause could never fire (the status was removed from `purchase_requests` by D68), so it was misleading dead code — Alexander confirmed the status is not coming back.
**How to apply:** Descriptions name statuses by their on-screen label ("Partially Received"), never the stored value or a paraphrase like "finished"/"terminal". New standing CLAUDE.md rule: changing a workflow's logic means re-reading and updating its description in the same session — a revision that leaves a stale description is not finished. Revision copy diffed clean (6/6 nodes identical) before editing; the condition node was rewritten with its full intended config, not patched.
**Affects:** workflows-explained.md §3.2/§4/§6 and tests/plan.yaml's header comment updated same session. Deliberately NOT changed: the create-budget guard's sibling filter still lists `cancelled` in `$notIn` — a harmless no-op not worth a revision. Also confirmed by Alexander: "Guard: Receive" and "Guard: PO Line Create" stay without an admin exemption (admin exemption is for cleanup/deletes, per D82's split). Two new test rules came out of this: R42 (D83's parked lines_total rule, reviewed + reworded — create/update is async and reloads the line, delete is sync and reads the payload FK; import path still manual-only) and R43, which closes a real gap — Guard A's BLOCK side had never been tested, only its pass side, despite workflows-explained.md §3.5 resting the committed_usd argument on it. R43 isolates the guard by editing `description` as procurement, whose PR update grant is unscoped and whitelists that field, so a deny can only be the guard.
**Two things found while verifying, both fixed same session:** (1) the revision silently appended " copy" to the title, which went live for ~40 minutes — a node-only diff reads clean and misses it; CLAUDE.md's revision-diff rule now names the workflow-level fields to compare. (2) Pre-existing D79 drift: Guard A was the only sync guard with `deleteExecutionOnStatus: [1]` (all three versions, so it predates this work) — set to `[]` on Alexander's approval; all 13 guards now compliant, and retained success rows (20) confirmed the fix.
**Status:** done + live-verified 2026-07-18. Suite 87/87 green (84 before R43's three cases). Guard A's block side proven by execution log: exactly the expected rejections at status -1, i.e. the deny came from the guard, not ACL. Rollback = disable `376198840057856`, re-enable `373846756098048`.

## D85 — Stranded project commitment is fixed by a director budget raise, never by auto-release (2026-07-18)
**Decision:** When a project-linked PO is closed, the purchase request's money stays committed. The way to free room on the envelope is for the **director to raise `budget_usd`** on the approved project. No mechanism releases `committed_usd` on PO close — not automatically, not by a procurement button. Scoped as MVP 018, low priority, not built.
**Why:** Two reasons. (1) Project-linked PRs skip the director and board entirely (Fork A), so the budget envelope is the director's only control over project spend — giving room back is a director-level act by definition. (2) Auto-release on PO close would hand the release lever to procurement, the same role that approves project-linked PRs: approve PR → issue PO → receive goods → close PO → release → approve the next PR, unbounded against one director-approved envelope, with nothing surfacing to the director. Raised by Alexander as a fraud question; the auto-release option was on the table and was dropped for exactly this.
**How to apply:** Never "fix" the stranded-commitment gap by adding a release path, a PR post-approval status, or a `commitment_released_at` flag. Deleting the approved PR to free budget is also out — it destroys the audit trail of a real, really-cancelled purchase. If a release ever becomes necessary, it needs a director approval task on each release, which costs more than the budget raise it replaces.
**Affects:** MVP 018 ([chunks/018-stranded-project-commitment.md](chunks/018-stranded-project-commitment.md)). Blocker found while scoping: an approved project is fully locked (D49/R26), so the director cannot currently raise the budget at all — that permission plus a budget-change audit trail is 018's actual build. Open question parked for build time: whether a raise crossing $15,000 must re-trigger board approval. Not addressed: the same root cause strands commitment when a PO completes under the PR amount or at a different FX rate. **Separate finding, closed 2026-07-18:** a Received PO can be walked back to Partially Received by correcting a line down. Checked against the live app — reaching Received has no side effects beyond the status field and a notification, so there is nothing to reverse. Receipt accuracy is a physical control, not a software one. No chunk needed.
**Status:** decided 2026-07-18, not built. No live change made. PRJ-26-0123 deliberately left at committed 4,900 / remaining 100 (PO-26-0468 closed as "No longer required") — under this decision that figure is correct, not drift.

## D86 — Production backups use NocoBase's Backup manager, not a cron/pg_dump/rclone stack (2026-07-18)
**Decision:** The server's backup design is the built-in **Backup manager** plugin, configured in the app UI: files-backup on, cron `0 3 * * *`, keep 7 locally, an encryption password set, and each finished `.nbdata` synced to a Backblaze B2 bucket added as an **Amazon S3** file storage. The hand-rolled `pg_dump` + `tar` + `rclone` cron stack is removed from the plan. On top of it, an occasional manual `nb backup create` from the Mac to iCloud stays.
**Why:** The plugin already covers all four requirements the cron stack existed for — database, uploaded files, schedule, off-site — in one restorable artifact. Verified live before deciding: `@nocobase/plugin-backups` is enabled, its settings are `scheduled`/`cron`/`keep`/`enableFilesBackup`/`storageId`/`encryptionPassword`, and its upload step ships the file to any file storage whose type isn't `local` (`managers/backup.js:363`). The free built-in `s3` engine in the image takes a custom endpoint, so B2 works **without** the paid S3(Pro) plugin. The cron stack was also the source of two real bugs (missed `storage/`, wrong container name).
**How to apply:** Three things the plugin does not cover, and they are why the manual Mac backup stays: (1) `compose.yaml` and `.env` are not in any `.nbdata` — copy them off the server by hand; (2) the encryption password is unrecoverable — store it off the server and off the bucket; (3) the app holds a B2 key that can delete, so scope the key to that one bucket and enable object lock/versioning. Restore drills go into a scratch same-version app, never into production — restore is a full replace. A `.nbdata` restores onto the same NocoBase version or newer, never older.
**Affects:** [docs/go-live.md](docs/go-live.md) §2.0 item 3–4, §2.4 (no `/opt/backups`), §4.1, §4.2 step 2, new §4.4 restore drill, and the final checklist — all rewritten this session. [docs/vps-setup-guide.md](docs/vps-setup-guide.md) gained §13.0 ("does the app back itself up?") as the first question, with §13.1–13.2's cron stack demoted to the fallback for apps that have no backup feature; §10, §14, §15.2, §15.3 adjusted to match. Unchanged: CLAUDE.md's per-chunk local `nb backup create` to iCloud, which is about the dev Mac, not the server.
**Status:** decided 2026-07-18, docs updated, nothing built — the B2 bucket and the plugin settings get configured during Part 4 of go-live. Not yet done: choose whether B2 object lock or lifecycle versioning, and confirm the free `s3` storage type against B2's endpoint in practice (endpoint support read from the shipped code, not yet exercised).

## D87 — PO lines freeze at issue: only receiving may change them (2026-07-18)
**Decision:** Once a purchase order leaves Draft, the only update allowed on its lines is a plain receiving entry. Quantity Ordered and Unit Price are fixed at issue; so is everything else on the line. New sync guard "Guard: PO Line Freeze — only receiving once PO issued" (`eiscjvwiqr6`, live version `376238895661056`, predecessor `376235116593152` disabled). Separately, `supplier`, `currency`, `fx_rate_to_usd`, `total` and `issued_at` were removed from `procurement`'s **create** field whitelist on `purchase_orders` (action row `373257175367682`) — D46 had already removed them on update.
**Why:** Document integrity, not budget. Once issued, the printed PO has gone to the supplier; if the stored line then moves, receiving and invoice matching compare against numbers that don't match the paper and nobody can say which is authoritative. **An earlier draft of chunk 019 justified this on budget grounds and was wrong** — Alexander caught it. The line budget-ceiling guard `c9c14tyn876` already caps the line sum at the PR's `quoted_total`, compares the newly submitted value, and never reads PO status, so over-budget edits were already rejected at every status. What was still open was moving the numbers *within* the ceiling (issue at 9,000 against a 10,000 PR, then edit to 9,999), and the same end state via import (D52).
**How to apply:** The guard asks "is this NOT a plain receive" — quantity present, or price present, or Received Quantity absent — and blocks on a non-Draft parent. Do not revert it to the narrower "is a quantity or price being set" test: that was the first version and it had a real hole (below). Admin/root exempt per D82. Created with `deleteExecutionOnStatus: []` per D79. No escape hatch by decision — if a supplier changes a price after issue, close the PO and generate a new one; a Revert-to-Draft button was discussed and deferred.
**The hole, found by testing:** the first version asked only "is a quantity or price being set". NocoBase renders an absent field and an explicitly-emptied one identically, so **blanking** a quantity slipped straight through, and so would a quantity change smuggled alongside a receiving entry. Caught by a case written specifically to probe the condition's shape (R45, drafted `# TODO verify` and expected to fail). Alexander chose the inverted rule — allow only receiving — over documenting the hole. **Generalisable: any guard testing `values.X != null` for "is X being changed" cannot see a blanking, and the sibling budget guard `c9c14tyn876` has the same shape** (harmless there — blanking a price only lowers the total).
**Affects:** Test rules R44 + R45 (7 new cases, suite 94/94). The four `purchase_orders` fixtures in tests/plan.yaml now create as **admin**, not procurement — the create-whitelist change silently drops supplier/currency/total, which left `po_issued` unable to pass the Issue guard; the product was right and the fixture encoded the old rule. [completed/019-po-execution-lock.md](completed/019-po-execution-lock.md) Phase 1b was already done by Alexander (Add new button on the PO table removed; one unparented dead `flowModels` row `i2sb9sjg84d` survives, cannot render). `purchase_orders.fx_rate_to_usd` and the `total_usd` formula field are **kept** — probably unused, cost nothing, may be wanted later.
**Status:** built + suite-green 2026-07-18. **Generate PO confirmed working by Alexander the same day, tested as Pat** — the create-whitelist change does not affect it, as predicted (workflow create nodes bypass ACL). Claude could not verify this from the CLI: Generate PO is a one-click custom action, `nb api` has no raw-HTTP verb, and the CLI session is root, which the guard exempts. Still pending: the freeze guard's own UI checks (go-live §1.2b). Rollback = disable `376238895661056` and re-add the five field names to the create whitelist.

## D88 — Every final decision notifies three people; the decider is never told (2026-07-19)
**Decision:** When a PR or a project reaches Approved or Rejected, an in-app message goes to the **submitter**, the **head of the submitter's department**, and the **head of Procurement**. Two exceptions: whoever made the decision is never notified, and Procurement is skipped when the record was rejected before it ever reached them. Returned — Info Requested still sends nothing. Built as 11 notification nodes across two new revisions: PR Approval `376242882347008` (47 nodes; 5 existing nodes re-targeted, 3 of them retitled, 5 new rejection nodes) and Project Approval `376252879470592` (28 nodes; 6 new). Predecessors `375705420496896` and `373589018214400` disabled. Supersedes D50's recipient rule.
**Why:** Rejections notified nobody at all — a rejected request died in silence and the submitter learned about it only by going and looking. Approvals told only the submitter. The decider-exclusion is noise control: they just clicked the button. Procurement is skipped on early rejections because the request never reached their desk. Rejection copy names the stage ("was rejected at the director stage") so a reader knows where it died without opening the record.
**How to apply:** Recipients are hardcoded per node, not resolved at runtime — each terminal node sits on exactly one approver's branch, so "who acted" is known from the node's position in the graph. No role checks, no extra condition nodes. Dept head = `{{$context.data.createdBy.mainDepartment.mainApproverId}}`; Procurement head = the first-running query node (`yrl9kgkrb3x` on PR, `ioqkg84ju36` on Project), which is why it resolves even at the earliest dept-stage rejection. No trigger changes were needed, so approval routing was untouched. Notification content stays walkthrough-only in tests/plan.yaml (line 71's standing convention — the runner cannot express it); do not add runner cases for message recipients.
**Affects:** MVP 017, the last build chunk before go-live. D50's recipient matrix superseded. Over-budget auto-rejection (`xtr5t4xy1gq`) now notifies all three rather than the submitter alone — Alexander's call, on the reasoning that Procurement should learn their approval was overturned; in practice the path is near-unreachable because linkage rules hide Approve when the amount exceeds remaining budget. Accepted duplicate: on the Skip Dept path the submitter *is* the dept head, so one person gets two identical messages. Email remains out of scope (needs SMTP + a template pass).
**The revision copy was unfaithful in three new ways this session, on top of D69/D75/D84.** (1) **A whole node vanished** — PR Approval's `x8s6jal11h9` "Write committed_usd to project", the node that books an approved PR against its project budget; the copy re-linked its upstream straight past it. Caught only by a node-count check (41 vs 42). (2) **`description` came back `null`** on both revisions. (3) **Three Project Approval nodes came back with rewritten logic** — Director Approval lost `returnTo: 'ik1ixug5rrs'` (the pointer sending a return back to Procurement; the button survived, its destination did not), both approval nodes had their `actions` arrays rewritten, and `amc633b85wx` had its department expression silently swapped to a different variable. All restored verbatim from the live version, keeping only the newly minted `approvalUid`/`taskCardUid`. **Standing rule: diff a revision by full node config and node count, not by the node keys the chunk happens to name** — a diff scoped to the nodes you intend to edit reads clean and misses all three of these. Not a loss, contrary to a mid-session reading: `categories` survives fine; `nb api workflow workflows get` just never returns that key.
**Also learned:** raw `resource create` on `flow_nodes` does **not** re-link neighbours — the new node knows its upstream and downstream, but neither neighbour points back, so both must be patched by hand. `flow-nodes duplicate` links correctly but works only within one workflow, which is why Project Approval's first node had to be created raw and the other five duplicated off it. `workflows revision` accepts `--title`, which pre-empts D84's " copy" bug at the source rather than catching it in the diff.
**Status:** built + live 2026-07-19, suite 94/94 green. The `committed_usd` repair is proven by the suite's own case ("project committed_usd written at approval, approved PRs only") passing against the new workflow — that case would have caught the dropped node independently. **Still pending: Alexander's UI walkthrough** (go-live §1.2b) — reject a PR at each of the five stages and a project, driving real approver users, and confirm the three approved-path titles now read correctly. Rollback = disable `376242882347008`/`376252879470592`, re-enable `375705420496896`/`373589018214400`, then `workflows sync --filter '{"enabled": true}'`.

## D89 — A guard must resolve the parent record itself, never read the submitted association value (2026-07-19)
**Decision:** "Guard: PO Line Create — block on terminal PO" (`polncreateg1`) now resolves the parent order id in a dedicated node at the head of the chain, and refuses the line outright when no parent can be identified. New live version `376333087145984`; predecessor `369880930516992` disabled and kept as history. Three intermediate versions built while debugging were deleted — two carried an always-block harness and were unsafe to leave lying around.
**Why:** The old guard filtered `purchase_orders` by `{{$context.params.values.purchase_order}}` — the submitted value, raw. NocoBase accepts two legal ways to name a parent: `purchase_order: 12345` and `purchase_order: {id: 12345}`. The first is a number and worked; the second handed Postgres a whole record, which it cannot cast to bigint (SQLSTATE 22P02), so the node threw and the request died with **HTTP 500 "Workflow on your action failed"** instead of a clean refusal. Every PO screen sends the plain-id shape, which is why nothing was ever seen in the UI, and why the test suite stayed green — the runner sends a bare id too. A direct API caller got a 500 on every line create.
**How to apply:** The head node `plc_pid` (formula.js) picks the first of `values.purchase_order.id`, `values.purchase_order`, `values.purchaseOrderId` that is greater than zero; `plc_hasid` then refuses with "Cannot determine which purchase order this line belongs to." when the result is 0, and only otherwise does the query run. **Do not restore the direct read.** Any new guard that needs a parent id gets the same head — reading an association value straight out of the payload only ever works for the one shape the UI happens to send.
**The formula.js trap that cost most of the session:** the engine does **not** substitute values into the expression as text — it replaces each `{{...}}` with a scope placeholder (`$$0`, `$$1`, ...) and binds the real values separately. So `VALUE('{{...}}')` evaluates the literal string `$$0`, and every quoted reference silently yields nothing. **Variables must be written unquoted** — `N({{...}})` — exactly as every other formula node in this project already does. A first attempt that quoted them read as valid, passed `flow-nodes test` against literals, and then allowed *every* create through in production. The corollary is also good news: because values arrive by scope and not by text splicing, an absent variable does not produce the empty-gap SyntaxError that the older notes warn about — `N()` of a missing value returns 0 cleanly, and `N()` of a nested record returns 0 too.
**Also learned:** `flow-nodes test` only sees the config you hand it — it has no `$context`, so it cannot catch a substitution bug at all; it validated an expression that was broken live. To debug a sync guard without writing rows, build a throwaway revision whose identify-check is forced false so every request is refused, then read the diagnostic node's result off the `jobs` table. And a node edit is refused the moment its workflow has executed once ("Nodes in executed workflow could not be reconfigured"), so each debugging iteration costs a fresh revision.
**The same fault was live in a second guard, found by checking on purpose.** "Guard: PO Line Create — budget ceiling (PR amount)" (`8u81nd3vxhc`) read the order the same raw way in **two** nodes — the order lookup `8ka4s7gcleo` and the existing-lines sum `huusodnuvgp`. New live version `376358445907968`; predecessor `370018742763520` disabled. Same head-node fix (`bud_pid`), no business rule touched, description left alone because what it checks did not change. **It was the more damaging of the two**, and it hid behind its own first condition: it only looks the order up once a quantity *and* a price are both present. Real lines always have both, so every realistic nested-shape create still returned a 500 even after `polncreateg1` was fixed — my first verification pass missed it purely because those probes omitted `unit_price`. **Lesson: verifying a fix against one guard proves nothing about the other guards on the same action.** The remaining four `po_lines` interception guards (`c9c14tyn876`, `mhfp4d15uee`, `f3dkb37te22`, `v61hc3ou3pa`) were checked and are clean — they read only scalar fields (`quantity_ordered`, `unit_price`, `received_quantity`), which have no shape ambiguity.
**Known expiry date on the fix.** The resolver converts the order id to a number, and NocoBase snowflake ids are bigints. Today that is safe with room to spare — live ids sit near `3.76e14` against a `9.007e15` limit, roughly **24x of headroom**, and every id resolved came back byte-exact (checked against the `jobs` results and the budget message's arithmetic). It is not permanent: if ids ever approach that limit the conversion starts rounding, the id match misses, and the guard **opens** rather than closing. The cross-project gotchas catalogue carried a blanket "never run a snowflake id through `N()`" warning that contradicted its own advice to normalize; it now records this measurement instead. Re-measure before assuming it still holds.
**Affects:** New test rule R50 with 6 cases (three refusals — plain id, nested record, no parent — plus a nested-shape allow on a draft PO, so the fix cannot read green by blocking everything). The nested case is the regression guard: HTTP 500 is not in the runner's `DENY_CODES`, so it fails loudly if the direct read ever returns. Confirmed independently by a parallel session hitting the same 500. The sibling guards `c9c14tyn876` (line budget ceiling) and `eiscjvwiqr6` (line freeze) read `values.*` in the same style and have **not** been checked for this shape — worth a pass before go-live.
**Status:** both guards built + verified live 2026-07-19. `polncreateg1` across all seven payload/status combinations; `8u81nd3vxhc` across three shapes under budget (all allowed), two over budget (both refused with the correct "130 USD over the approved 120 USD" wording, which only renders if both lookups resolved), and a closed-PO create (still refused by `polncreateg1`). Test debris created during the failed attempt (12 po_lines, 3 of them on closed PO-26-0468) deleted with Alexander's approval. **Suite green 109/109 on 2026-07-19**, merged to master and pushed. The rule was drafted as R46 and renumbered to **R50** at merge time: the chunk-020 session was concurrently drafting its own R46/R48/R49 in the main working tree, uncommitted, so both branches had claimed R46 for different rules. R47 was left alone — chunk 020 skips it, apparently reserving it for the print-watermark rule. Rollback = re-enable `369880930516992` + `370018742763520`, disable `376333087145984` + `376358445907968`, then `nb api workflow workflows sync --filter '{"enabled": true}'`.

## D90 — Second maintenance pass: debris cleared, docs refreshed, trap catalogue consolidated (2026-07-19)
**Decision:** Second nb-maintain pass, all fixes approved by Alexander. Live: deleted the 2 `[TEST] close trigger` purchase orders (`375708096331809`, `375758696415266`) and pruned 3 workflow revisions to D81's keep-current-plus-2 depth (oldest predecessors of PR Approval `375674676248576`, Project Approval `372389893963776`, receiving recompute `368072534523904`). Files: 13 superseded runner reports removed; 2 merged clean worktrees + branch `claude/optimistic-zhukovsky-cf8141` removed. Docs: HANDOFF.md rewritten to current state, notes.md stale sections swept, roadmap 020 status corrected, `workflows-explained.md` caught up with D83/D87/D89 and the 020 receiving roll-up.
**Why:** the pass found HANDOFF three days / ~15 commits stale (second time in a row), stale drafting narrative in notes.md, and ~45 cross-project NocoBase traps living as full auto-memory entries while gotchas.md — the declared single home — lacked most of them.
**How to apply:** two standing changes. (1) CLAUDE.md session-end checklist now includes refreshing HANDOFF.md — a session that changes project state and leaves HANDOFF behind is not finished. (2) Auto-memory no longer carries NocoBase mechanism traps: 46 entries were merged into `nb-bootstrap`'s `references/gotchas.md` (new entries + a Template print section) and retired from memory; the memory index now holds 5 Alexander/project facts plus one pointer to the catalogue. Add future traps to gotchas.md directly.
**Affects:** every future session (leaner memory load, one trap home); next maintenance pass (revision pruning rule unchanged, D81).
**Status:** done 2026-07-19. Pre-deletion backup `havenbeheer_2026-07-19_1718.nbdata` on iCloud. Escalations still parked: 14-guard consolidation question, R42 bulk-import manual check, D80 runner-teardown orphans.

## D91 — Chunk 020 complete: value floors live on the fields; the May-era minimum-1 FX rule replaced (2026-07-19)
**Decision:** Chunk 020's value-bound sweep is live as server-side field validation: `quantity_ordered` > 0, `unit_price` ≥ 0, `budget_usd` > 0, `fx_rate_to_usd` > 0 when present (empty allowed), joining the earlier `received_quantity` ≥ 0. Two rules already existed, UI-set around May: `quoted_total` "greater than 0" — kept, stricter than the drafted ≥ 0 and right — and `fx_rate_to_usd` "minimum 1" — **replaced with "greater than 0" on Alexander's approval**, because EUR is an offered currency and a euro rate (~0.92 local-per-USD) would have been refused.
**Why:** None of these floors was enforced anywhere; a negative price silently lowers the lines total the budget ceiling checks, and a zero rate makes USD totals infinite. Field-level validation covers routes no guard sees (sub-tables, imports, workflow writes) and binds admin too.
**How to apply:** Record-local bounds go in `field.options.validation`; guards stay for rules needing related records, status, or roles (already a 020 chunk decision). The refusal messages use the fields' on-screen labels ("PO Lines: Quantity Ordered must be greater than 0") — no friendlier client-side layer needed. Never restore a rate floor at or above 1.
**Affects:** tests/plan.yaml R51 (7 cases; the 0.92 case exists to block a minimum-1 comeback). Chunk 020 closes: watermark confirmed by Alexander on a printed draft, receiving roll-up and negative floor were already green. docs/user-guide.md Stages 6–7 updated.
**Status:** built + probed live + suite green 116/116, 2026-07-19. Rollback = clear the five `validation` keys on the fields (received_quantity's stays, it predates this).

## D92 — §1.2b verification walkthroughs delegated to Claude, executed via API as real users (2026-07-19)
**Decision:** On Alexander's instruction ("I don't want to waste time on these long walkthroughs, you should be able to do this"), the outstanding §1.2b checks for chunks 017 and 019 were executed by Claude through the API instead of Alexander clicking through the UI. A one-off walkthrough script drove 9 scenarios as the real approver accounts (Oliver, Pat, Dana, plus a temporary Operations-department submitter): all five PR rejection stages, project rejection at dept and director stage, and the two approved-to-final paths. Recipients were read back from the in-app message records (`notificationInAppMessages`) and matched the D88 matrix exactly in all 9 — actor never notified, procurement skipped when the record never reached them. 019's boxes were closed against suite rules R44/R45 (real signed-in users, not root). Suite re-run after teardown: 116/116.
**Why:** the notification recipients were the one thing the runner deliberately doesn't check (plan.yaml line 71); everything else in the walkthroughs duplicated already-green suite cases. The API path drives the same requests the UI buttons send.
**How to apply:** future "reject/approve at every stage and check who's told" verifications can be scripted the same way — real approver sign-ins, temp submitter in a department whose head is NOT Pat (so dept head ≠ procurement head and the recipient slots are distinguishable), read `notificationInAppMessages` back, delete everything created including the messages. Reserve Alexander's manual checks for things only the screen shows (a button he hasn't built, a print layout).
**Affects:** chunks 017, 019, 020 (all close); go-live §1.2b (ticked); the test-gate convention — "user verbally confirms UI" is satisfied by delegation for API-reachable behavior.
**Status:** done 2026-07-19. All walkthrough records, messages, and the temp user deleted; suite green 116/116 after.

## D93 — Standing verification split: Claude runs every API-reachable check; Alexander only gets screen-only items (2026-07-19)
**Decision:** What D92 did once is now the standing way of working, project-wide and suite-wide. Every walkthrough, verification, or test that can be driven through the API — real signed-in users, real sequences, results read back from the data — Claude executes itself, without asking. Alexander is asked only for **screen-only** checks: print layouts, button visibility/placement, form appearance, anything only the rendered page shows — named explicitly, one by one. He may still spot-check manually whenever he chooses, but chunk completion no longer waits for manual confirmation of behavior the API already proved.
**Why:** Alexander: "All the walkthroughs and tests that you can do yourself with the API you should do. Only ask me for screen-only checks and validations." The API path sends the same requests the UI buttons send, so it verifies the same behavior; his time goes to what only eyes can judge.
**How to apply:** Test gate rewritten in this project's CLAUDE.md and in `nb-bootstrap`'s CLAUDE.md.template + chunk.template.md (acceptance criteria now name the verification method per item); nb-test SKILL.md scope notes the one-off-walkthrough pattern. Pattern reference: D92 + `snippets/walkthrough-approval-notifications.py`.
**Affects:** every future chunk here and in new NocoBase projects; go-live §1.2-style checklists (API-reachable boxes are Claude's to close, screen-only boxes are Alexander's).
**Status:** in force 2026-07-19.

## D94 — Guard payload-shape audit closed: one fault found and fixed, eleven guards cleared (2026-07-19)
**Decision:** "Guard: Create PO (PR must be approved)" now resolves the source purchase request itself at the head of the chain instead of reading the submitted value raw, and refuses the create outright when no request can be identified. New live version `376416662847488`, predecessor `366562380808192` disabled and kept as history. The rest of chunk 021's sweep found nothing: every other enabled interception guard is clean.
**Why:** The guard filtered `purchase_requests` by `{{$context.params.values.purchase_request}}` — the D89 fault, in the oldest guard in the set (26 May). Confirmed live before any change, against the one rejected PR in the system so a healthy guard would refuse and write nothing: plain id → clean 400 with the business message; `purchase_request: {id: N}` → **HTTP 500 "Workflow on your action failed"**; `purchaseRequestId: N` → 400 but for the wrong reason, the lookup finding no row and the guard concluding "not approved". That third shape is the quieter half of the fault: an *approved* PR named that way would have been refused a PO.
**How to apply:** Head node `cpo_pid` (formula.js, variables unquoted per D89) picks the first of `values.purchase_request.id`, `values.purchase_request`, `values.purchaseRequestId` that is greater than zero; `cpo_hasid` refuses with "Cannot determine which purchase request this PO is for." when the result is 0; the existing query `ww4mxz67ge8` now filters on `{{$jobsMapByNodeKey.cpo_pid}}`. No business rule changed — an approved PR without a PO is still the only thing accepted. Alexander chose refuse-on-unidentifiable, same reasoning as `polncreateg1`: a PO belonging to no purchase request is meaningless.
**What the sweep cleared, so nobody re-audits it:** 14 enabled interception guards read. `c9c14tyn876` and `eiscjvwiqr6` — the two D89 explicitly left unchecked — are **clean**; they read only `quantity_ordered`, `unit_price` and `received_quantity`, scalars with no shape ambiguity. Both PR Budget guards (`lylobzvlh5p`, `ebq41ibq60r`) already carry a resolver head from earlier work, so the PR-side associations the chunk feared most were never exposed. `2h75zryz3cb`, `xvcsdv07c5j`, `f3dkb37te22`, `v61hc3ou3pa`, `496ookqmg01` read nothing from the request payload at all. `b6brl8r9c58` and `mhfp4d15uee` are scalar-only. `polncreateg1` and `8u81nd3vxhc` still carry their D89 resolver heads at the expected versions.
**Self-inflicted detour worth recording: `workflows revision --filter-by-tk` forks the lineage.** Revising by record id minted a brand-new key (`py19meuuilk`) instead of continuing `vgv8hcrtjvx`, exactly the stray-copy failure the gotchas catalogue already warns about for `$and`-wrapped filters — the catalogue says to pass `--filter '{"key":"<key>"}'` at top level, and I passed `--filter-by-tk` without checking it first. `workflows update` then ignored a write to `key`, as the catalogue also says it does. Recovered without re-editing a single node: revised *from the stray*, forcing the lineage key in the body — `--filter '{"key":"py19meuuilk"}' --body '{"key":"vgv8hcrtjvx",...}'` — which the revision endpoint honors. Node chains diffed byte-identical, 8 nodes each. Final live version is **`376416662847488`** under `vgv8hcrtjvx`; the stray `376415305990144` was deleted with Alexander's explicit approval the same day, along with its 15 executions — **NocoBase does not cascade those**, so deleting a workflow leaves its execution rows orphaned and pointing at a workflow that no longer exists; delete them by `workflowId` as a second step. Passing `--title` on both revisions prevented D84's " copy" suffix. **Lesson: check the returned `key` against the source lineage before building anything on a revision** — the whole detour cost was building on an unverified one. **Rollback:** re-enable `366562380808192`, disable `376416662847488`, then `nb api workflow workflows sync --filter '{"enabled": true}'`.
**Affects:** New test rule **R52** with 5 cases and two new fixtures (`pr_approved_6`, `pr_approved_7` — PR→PO is 1:1, so each allow case needs its own approved PR). Three denies (nested, foreign-key, no-PR-named) plus **two allows, one per shape** — the allows are the load-bearing half: a resolver returning a wrong non-zero id would find no row and refuse with the same "must be approved" message as the denies, so denies alone cannot tell a working resolver from a broken one. Suite **121/121 green 2026-07-19**. `workflows-explained.md` gained the Create PO row its own prose already referenced.
**Status:** built + verified live 2026-07-19 — all four payload shapes re-probed against the final live version (three refusals with the right message, one with the new cannot-determine message), then both allow shapes proved by the suite. Chunk 021 complete; the last pre-go-live build item is closed.

## D95 — Pre-go-live data cleanup: business records wiped, counters reset, master data kept (2026-07-20)
**Decision:** The local app was emptied of all business data so the go-live backup ships a clean system. Deleted: 9 purchase requests, 5 purchase orders, 13 PO lines, 3 projects, 1 PR comment, 5 uploaded files, 634 in-app notifications, and the whole workflow execution history — 5,885 rows plus ~1,460 job rows (5,706 via the API, the last 179 by SQL, see below). Kept: 3 suppliers, 4 departments, 6 units of measure, 2 delivery addresses, and all 13 users — the 6 named personas and the 6 `[TEST]` accounts, which stay so the suite still runs against production-shaped config. Chunk 016 closed the same day on Alexander's B8 confirmation, so no build chunk remains open except 018 (low priority, not a blocker).
**Why:** Alexander is restoring this backup onto the VPS. Dev fixtures and five years of test approvals would land in front of real users as if they were real orders. Master data survives because departments carry the approval routing and the rest is genuinely reusable.
**How to apply:** Delete children before parents (notifications → comments → PO lines → POs → PRs → projects), as admin — every lock guard exempts admin (D71/D82), so completed and closed records delete too. **Two API traps found doing this:** `resource destroy --filter-by-tk` silently no-ops when handed more than ~50 ids (it returns success and deletes nothing), so batch in 50s or use `--filter '{"id":{"$gt":0}}'`, which is far faster anyway. And **a `status = 0` (started) execution cannot be deleted through the API at all** — `executions destroy` returns `{"data": 0}` and leaves the row, while `executions cancel` 500s with "Cannot destructure property 'approval' of 'approvalExecution' as it is null" once the approval record is gone. 179 such rows survived the API purge (the D80 backlog). They can only be removed with a scoped SQL delete straight against Postgres (`delete from jobs where "executionId" in (select id from executions where status = 0)`, then the executions themselves), which is safe here: no foreign key anywhere references `executions` or `jobs`, every approval table is empty, and every user's approval-todo counter already reads `{"pending":0,"all":0}`, so no one sees a phantom pending approval. **Alexander ran this himself** — Claude Code's permission classifier blocks a `DELETE` against the live database, correctly, and that block was not worked around. Verified afterwards straight from Postgres: `executions` 0, `jobs` 8. Those 8 are older orphans (4 executions deleted earlier by the API, whose job rows it left behind) — invisible, since the UI only reaches a job through its execution.
**Record numbering:** `purchase_requests.pr_number` sat at 772 and `projects.project_number` at 228; both were reset to 0 in the `sequences` table, so the first live records read **PR-26-0001** and **PRJ-26-0001**. Safe because every record that used the old numbers is deleted. `purchase_orders.po_number` has no counter — it is a plain text field the Generate PO workflow fills by copying the request's number, so it follows automatically. Two dead `po_number` sequence rows (keys 37231, 29563) survive from an older design; nothing reads them and they were left alone.
**Rollback:** `havenbeheer_precleanup_2026-07-19_2339.nbdata` in the iCloud backups folder, taken before the first deletion.
**Affects:** `docs/go-live.md` §1.5 (the final backup is now taken from an empty app), the suite (still 121/121 green after cleanup — it seeds and tears down its own fixtures), and any future session expecting dev data to browse: there is none, by design.
**Status:** done 2026-07-20. Every business collection verified at zero; suite re-run green against the emptied app.
