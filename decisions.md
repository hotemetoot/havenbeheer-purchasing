...
**Status:** built + structure-verified 2026-07-04; **pending user activation**, then E2E via the projects rules' first run.
...
**Status:** rule text approved 2026-07-04; ACL + approval-process build pending; R26 carries `# TODO build+verify`.
...
**Status:** term decided (`completed`) 2026-07-04; rename build pending; R27 carries `# TODO build+verify`.
**Related (D69):** a parallel "target date field doesn't exist under either name" gap was found on `purchase_orders` (no `completed_at`/`closed_at` field, yet the Complete PO workflow + PO ACL reference them) — see D69. When building this projects rename, run the same field-existence check on both collections.
...
**Related finding, NOT part of this change (verify separately):** while dropping the columns, found that `purchase_orders` has **no `completed_at` and no `closed_at` field** (only date fields: `createdAt`, `issued_at`, `payment_date`, `updatedAt`). Yet the **Complete PO** workflow (`368971625791488`) writes `{status: completed, completed_at: now}` and procurement's PO view/update ACL whitelists list `completed_at`, `closed_at`, `close_comment`, `close_reason`. So completing a PO likely does **not** persist a completion timestamp (silent unknown-attribute drop). No completed/closed PO rows exist yet, so this is unobserved, not confirmed-broken. Same class of "target date field doesn't exist under either name" as D67's projects rename — apply the same existence check to POs when addressing either.
...
**Status:** live changes done + verified 2026-07-04. R17 word-review + the PO `completed_at`/`closed_at` gap pending Alexander.
