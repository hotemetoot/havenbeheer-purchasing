...
**Affects:** New test rule R46 with 6 cases (three refusals — plain id, nested record, no parent — plus a nested-shape allow on a draft PO, so the fix cannot read green by blocking everything). The nested case is the regression guard: HTTP 500 is not in the runner's `DENY_CODES`, so it fails loudly if the direct read ever returns. Confirmed independently by a parallel session hitting the same 500. The sibling guards `c9c14tyn876` (line budget ceiling) and `eiscjvwiqr6` (line freeze) read `values.*` in the same style and have **not** been checked for this shape — worth a pass before go-live.
