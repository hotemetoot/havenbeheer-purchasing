...
- Client-facing docs, grows per chunk: [docs/user-guide.md](docs/user-guide.md) — Step 7 backfill nearly done. Written: §1–§3 reference + Stage 1 (create/submit) + Stage 2 (full approval ladder) + Stages 3–7 (PO create → issue → receive → complete/close → print). Remaining stub: **Appendix — Suppliers** only. Throughout, `📷` markers flag screenshots the human still adds. Write from the live app, not plans; match exact app labels.
...
- Kept non-standard folders (organically grown pre-retrofit, deliberately not restructured — real, load-bearing content with no better home in the standard tree): [Planning and Design/](Planning%20and%20Design/) (design validation docs), [nocobase docs/](nocobase%20docs/) (doc cache), [Outputs/](Outputs/), [snippets/](snippets/) (reusable JS, e.g. `po-line-items-total.js` — RunJS isn't version-controlled, so this is the source of truth for it), [templates/](templates/) (PO print-template source, `build_po_template.py`), [archive/](archive/), `backups/` (gitignored — `nb backup create/restore` output), [role-acl-guidelines.md](role-acl-guidelines.md) (standalone ACL reference, kept at root rather than folded into `design/permissions.md`), [workflows-explained.md](workflows-explained.md) (plain-English reference for Alexander: how the PR/project approval ladders, guards, committed_usd mechanism, and batch-vs-one-by-one update modes work — written from live node reads; any D-entry that changes a workflow should also update this file).
...
A chunk isn't complete until `nb-test run` is fully green AND the chunk's behavior has been verified end-to-end. **Verification is Claude's job wherever the API can reach it** (Alexander's standing instruction 2026-07-19, D92/D93): drive the real flows as the real signed-in users — approver accounts, a temp submitter where recipient slots must stay distinguishable — and read the outcome back from the data (e.g. `notificationInAppMessages` for notification checks). When the suite deliberately doesn't cover something, script a one-off walkthrough (pattern: `snippets/walkthrough-approval-notifications.py`), tear down everything it created, and re-run the suite after. Ask Alexander only for **screen-only checks**: print layouts, button visibility/placement, form appearance, anything only the rendered page shows. Name those items explicitly and one by one — never hand him a broad "check the UI". He may still spot-check anything himself when he chooses, but chunk completion doesn't wait on checks the API already proved. Once the backend is tested green, write a descriptive interface brief into the chunk file (blocks, field bindings, actions, linkage suggestions) and stop; the human builds the screen. The brief is a suggestion sheet, not a contract — the result may differ completely.
...
- Update HANDOFF.md — "Where things stand", in-flight chunk statuses, open items. It is the cross-session entry point and two maintenance passes in a row found it stale; a session that changes project state and leaves HANDOFF behind is not finished.
...
## NocoBase rules
- **A workflow's description is part of the workflow.** Every workflow carries a category (Guards / Approval / Actions / Recompute) and a plain-English description of what it checks and writes, stored on the workflow record itself — that's their home, not a doc. Whenever you change a workflow's logic, thresholds, statuses, or guard conditions, re-read its description in the same session and update it if it no longer matches. A revision that leaves a stale description is not finished. Describe statuses by their on-screen labels ("Partially Received"), never the stored value (`partially_received`), and never a paraphrase like "finished" or "terminal". Same rule when a status enum changes: sweep the descriptions that name the removed value.
...
- **Diff every revision copy before enabling it.** The revision copy is not always faithful — it has silently dropped condition clauses twice (D69, D75). Compare the new version's node configs against the source node by node; when you're editing a node anyway, write the full intended config instead of trusting the copy. **Diff the workflow-level fields too, not just the nodes** — `title`, `options` (`deleteExecutionOnStatus`, `stackLimit`), `sync`, `description`, `categories`. The revision silently appends " copy" to the title, which then ships as the live workflow's name (hit 2026-07-18, D84); a node-only diff reads clean and misses it.
- **Every new interception guard gets the admin-exempt head by default:** two nodes in front of the lock check — look up whether the caller holds root/admin, run the block only when they don't (the D71/D63 pattern). Keeps cleanup and test teardown possible without disabling guards.
- **Every new sync guard keeps its execution history:** create it with `deleteExecutionOnStatus: []` (the "delete on success" setting empty), or the dispatcher double-processing race returns 500s under load (D79). History is prunable later.
