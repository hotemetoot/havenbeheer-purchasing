...
- **Diff every revision copy before enabling it.** The revision copy is not always faithful — it has silently dropped condition clauses twice (D69, D75). Compare the new version's node configs against the source node by node; when you're editing a node anyway, write the full intended config instead of trusting the copy.
