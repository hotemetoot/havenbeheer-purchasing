...
- **MVP9d — backend DONE, Complete button BUILT, verification still PENDING.** Complete workflow (`qh7b3hc5q1r`, now D34 active ver `368971625791488`), broadened Close (`f8gpu17s6hq`), and the two immutability guards (`xvcsdv07c5j` PO header, `f3dkb37te22` po_lines) are live + enabled. The **Complete button** (`e60dce0bb2d`) is now built on PO detail block `g9xffr68350`. **D34 (2026-06-09):** Complete now requires an attached invoice + positive `total_usd` (guard + button linkage). **What's left:** end-to-end manual C1–C5 (incl. the new D34 cases: received PO with no invoice → blocked; with no USD total → blocked; with both → completes). Then a "MVP9d verified" commit.
- **PO surface drift (FYI):** the user rebuilt the PO detail block — the doc's Close button `lylrxwl1b3g` is gone (removed on purpose), and a bare **`TemplatePrintRecordActionModel` ("Template print", uid `c579329db0d`)** now sits beside Send. **MVP9e is NOT started** — it's just an empty button: no print template configured, nothing tested. Treat 9e as pending. Capture 9e surface IDs when that MVP is actually built.
