...
- **Verified 2026-06-13:** `flow-nodes test` (completed/closed→block, draft/empty→allow); live direct-create probe (completed→block, draft→line created); live **association**-create replay with `purchase_order` in values → completed PO **blocked**. Remaining client-side check (user): confirm `{{ ctx.popup.record.id }}` resolves at form-submit time via one real add-line click. D24 bulk-create caveat: a bulk/`filter.$in` create isn't intercepted.
