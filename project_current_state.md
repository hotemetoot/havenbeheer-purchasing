...
- **Role hardening COMPLETE (2026-06-12).** All HANDOFF.md phases closed (D38–D41); HANDOFF.md deleted. Follow-up status after the late session (D42):
  ...
  - **STILL OPEN — Scopes 3, 6–9** (`rolesResourcesScopes`, snake_case FK columns): only referenced by 8 **orphaned** `dataSourcesRolesResourcesActions` rows (ids in `backups/acl-scopes-3-6-9-orphans-20260612.json`; parents gone, rows inert). Plan: delete the 8 orphan rows + the 4 scope rows (or camelCase-fix the scopes). **Permission-blocked 2026-06-12** — needs user to run/approve.
