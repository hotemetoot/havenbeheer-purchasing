...
- **Role hardening COMPLETE (2026-06-12).** All HANDOFF.md phases closed (D38–D41); HANDOFF.md deleted. Low-priority follow-ups carried over from it (user-approved "afterwards", none blocking):
  - `po_lines` titleField → `description` (m2o displays show raw IDs otherwise).
  - Workflow executions retention (371 executions at the 2026-06-11 audit) — prune or configure retention.
  - **Scopes 3, 6–9** (`rolesResourcesScopes`) contain snake_case FK columns (`department_id`, `submitter_id`) and will 400 if ever bound — fix to camelCase like scope 2, or delete.
  - **D39 follow-ups (draft stage removed by user):** the Cancel workflow guard (`kj9d7bnvw02`, status=="draft") never matches → Cancel is effectively dead for PRs; scope 2's editable window is effectively only `info_requested`. Ask the user whether cancel-before-decision / edit-own-pending should exist; adjust the Cancel workflow condition and scope 2 accordingly. The status enum still contains `draft` and the field default is still `'draft'` — verify how "submit directly" actually sets status before touching either (the PR Approval root update node sets status anyway).
  - Pre-existing procurement `attachments`/`suppliers` ACL rows reference dangling scopeId `363334209503233` (behaves as all-rows) — optionally re-apply with explicit null/all scope.
  - **Smoke test on next PR creation:** the trigger appends were trimmed (D41) on the active version before any execution — the first real PR submission confirms the trigger is healthy (expected: dept stage assigns normally).
  - Board surface has Approve + Reject but no **Return** action (user added Reject 2026-06-12; Return presumed intentionally absent — confirm if board-returns are ever wanted).
