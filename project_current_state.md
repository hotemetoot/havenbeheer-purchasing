...
- **D47 (PO line pricing + per-line budget block) — CORE BUILT + CLI-VERIFIED 2026-06-14; UI PENDING.** Data model + 4 new workflows + `issue_po` revision all live & enabled (see D47 in decisions.md and the workflow sections). **Stale: old `issue_po` version `369914017284096`** (retired; current = `370019772465152`). **Still TODO (UI, needs the user / nocobase-ui-builder):** (1) drive the **Issue button** to confirm the all-priced gate live; (2) surface `needs_reprint` on the PO detail (linkage indicator when true) + add a **"Mark reprinted"** action button (update `needs_reprint=false`, visible when true); (3) add `unit_price` + `line_total` columns to the PO detail lines block, and `unit_price`/`line_total`/`lines_total` to the **print template** (`templates/build_po_template.py` → docx → copy to `storage/print-templates/po-template-mvp9e.docx`). **Cleanup:** one labelled orphan test PR `370020208672768` ("TEST budget guard", approved, no PO) couldn't be deleted (PR immutability guard locks approved PRs; declined to disable the guard) — clear it at the next fresh-slate wipe.
