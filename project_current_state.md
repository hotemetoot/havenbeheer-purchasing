...
- **Key:** `cv237r8h7k9`
- **Active version ID:** `369166026080256` (enabled=true, current=true) — **a user revision found live 2026-06-11** (audit re-query); 30 nodes, all branches verified intact, same structure as the D37 revision below. Approval surfaces on THIS version: Procurement `su56vx8p048`, Dept `nwjvklrknlf`, Director `7kuv49goy4o`, Custom `h2sh3r9monu`, Board `xwd019rr78k`; all 10 referenced CommentFormModels exist. Hardcoded assignees still present: Director node `sxvxwl498xg` → [12], Board node `01upqmcb1qy` → [11] (Phase 2 makes these data-driven — see HANDOFF.md). **Board surface has only an Approve action (no Reject/Return buttons)** though the workflow has board reject/return outcome nodes — confirm with user whether intentional.
...
(`fiona.finance` — Finance approver test user — planned in Phase 2, see HANDOFF.md.)
...
- **finance**: still `null` strategy, no users — Phase 2 adds fiona.finance + view/render-enabler grants.
...
### Workflow versions of `cv237r8h7k9` (all disabled before the active `369166026080256`):
...
- **Role hardening remaining work:** Phases 2–4 + low-priority items are fully specced in [HANDOFF.md](HANDOFF.md) — start there next session.
